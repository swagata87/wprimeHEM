...
int Plot_mT_Stage1() {
  ...
  mT_Stage1_DYJetsToLL_M50->Rebin(20);  
  ...
  */
  ...
  TLegend *leg_example = new TLegend(0.75,0.70,0.94,0.94);
  ...
  TCanvas* ratio_c = new TCanvas("ratio","ratio_canvas",600,600);
  //ratio_c->cd();
  ratio_c->Divide(1, 2);

  //  (TPad*)ratio_c_1->SetPad(0., 0., 0.88, 1); 
  // (TPad*)ratio_c_2->SetPad(0.89, 0, 1, 1 ); 
  ratio_c->cd(1);
  gPad->SetPad(.005, .2, .995, .995);
  /*
  TPad* canvas_up = (TPad*) ratio_c->GetListOfPrimitives()->FindObject("ratio_c_1");
  TPad* canvas_dw = (TPad*) ratio_c->GetListOfPrimitives()->FindObject("ratio_c_2");
